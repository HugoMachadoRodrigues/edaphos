...
**NOT** open a public GitHub issue.  Instead, send a private email
to:
...
* Reproduction steps (a minimal example or pointer to the
  affected function).
* The version of `edaphos` you tested against
  (`packageVersion("edaphos")`).
* Your assessment of severity (e.g. CVSS-style: low / medium /
  high / critical).
...
* Code in this repository (`R/`, `src/`, `inst/`, `data/`,
  `tests/`).
...
* Vulnerabilities in dependencies (Rcpp, RcppArmadillo, torch,
  terra, dagitty, bnlearn, ranger, ...).
...
attribution in the patch release notes will be named (with
permission).  Anonymous reports are equally welcome and will be
