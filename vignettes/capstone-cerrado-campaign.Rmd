...
knitr::kable(
  ...
  col.names = c("Pilar", "Método", "CRPS", "PICP (90%)",
                "MPIW (90%)", "RMSE"),
  caption = paste0(
    "Diagnóstico unificado de incerteza para os seis pilares do edaphos ",
    "no conjunto de dados do Cerrado. CRPS: menor é melhor. ",
    "PICP: próximo de 0,90 é ideal. MPIW: menor indica maior resolução."
  )
)
```

```{r reliability-plot, fig.height = 5.5, fig.cap = "Curvas de confiabilidade (reliability diagrams) dos seis pilares. A diagonal em 45° representa calibração perfeita. Pilares acima da diagonal são conservadores (intervalos muito amplos); abaixo são otimistas (subcobertura)."}
rel_list <- R$reliability_list

rel_df <- bind_rows(lapply(names(rel_list), function(nm) {
  d <- rel_list[[nm]]
  # field 'coverage' or 'empirical' depending on source
...
ggplot(rel_df, aes(x = nominal, y = coverage, color = pilar)) +
  ...
  labs(
    ...
    title = "Calibração cruzada: seis pilares do edaphos"
  ) +
  theme(legend.position = "right")
```

```{r calibration-radar, fig.cap = "Resumo radar dos quatro diagnósticos por pilar (valores normalizados para 0–1, onde 1 = melhor desempenho). O Pilar 1 tem o CRPS mais baixo absoluto; o Pilar 5 apresenta o melhor balanço geral para a decisão de amostragem."}
# Fallback bar chart (works without fmsb)
norm01 <- function(x, invert = FALSE) {
  r <- (x - min(x, na.rm = TRUE)) / (max(x, na.rm = TRUE) - min(x, na.rm = TRUE) + 1e-9)
  if (invert) 1 - r else r
}
radar_scores <- data.frame(
  pilar    = cal_tbl$pilar,
  CRPS     = norm01(cal_tbl$crps, invert = TRUE),
  PICP_err = norm01(abs(cal_tbl$picp - 0.9), invert = TRUE),
  MPIW     = norm01(cal_tbl$mpiw, invert = TRUE),
  RMSE     = norm01(cal_tbl$rmse, invert = TRUE)
)
cal_long <- tidyr::pivot_longer(radar_scores, -pilar,
                                 names_to  = "métrica",
                                 values_to = "score_norm")
ggplot(cal_long, aes(x = métrica, y = score_norm, fill = pilar)) +
  geom_col(position = "dodge", width = 0.7) +
  scale_fill_brewer(palette = "Dark2", name = "Pilar") +
  scale_y_continuous(labels = scales::percent, limits = c(0, 1)) +
  labs(
    x     = NULL,
    y     = "Score normalizado (1 = melhor)",
    title = "Diagnóstico multi-métrica: seis pilares (normalizado 0–1)"
  ) +
  theme(legend.position = "right",
