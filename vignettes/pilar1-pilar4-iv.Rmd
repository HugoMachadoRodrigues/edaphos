...
- **v1.9.0 (atual)**: estimador 2SLS completo + diagnósticos (F,
  Sargan) + adaptador para `edaphos_posterior` + benchmark com
  *proxy embeddings* em 1 095 perfis + validação em DGP sintético.
- **v1.9.1**: extração real de embeddings MoCo v2 nas localizações
  WoSIS (requer o encoder v2 e os raster stacks SoilGrids / WorldClim
  / SRTM); re-executar o benchmark. **O teste decisivo** é se o
  Sargan passa a não-rejeitar.
- **v1.9.2**: análise de sensibilidade à la @CinelliHazlett2020 — quão
  forte precisaria ser a violação de exclusão para mudar qualitativamente
  a conclusão?
