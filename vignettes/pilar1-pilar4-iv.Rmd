...
precisaria ser um confundidor latente $U$ — em termos de
$R^2$ parcial com $X$ e $Y$ — para zerar o efeito estimado?*

A fórmula-chave:

$$
|\mathrm{bias}|_{\max} = \mathrm{SE}(\hat\beta) \cdot
  \sqrt{\frac{R^2_{Y\sim U | X, Z} \cdot R^2_{X\sim U | Z}}{1 - R^2_{X\sim U | Z}}}
  \cdot \sqrt{\mathrm{df}}
$$

O **Robustness Value (RV)** é o $R^2_{U} = R^2_{Y\sim U} = R^2_{X\sim U}$
que, assumindo *equal confounding* na ambas as pontas, zeraria o
efeito.  **RV baixo = efeito frágil**.

```{r sens-load, eval = FALSE}
...
Running `data-raw/causal_sensitivity_run.R` on all four estimators
for the three Cerrado exposures:
...
1. **Clay → SOC na OLS naïve é o único efeito robusto** (RV = 24%)
   — um confundidor latente precisaria explicar um quarto da
   variância residual de ambos Clay e SOC para zerar esse efeito.
   Consistente com a literatura: clay é o preditor mais direto de SOC
   e pouco confundido.
...
O bloco abaixo produz o plot clássico de @CinelliHazlett2020 — um
grid 2-D com $R^2_{X\sim U | Z}$ no eixo-x, $R^2_{Y\sim U | X, Z}$
no eixo-y, e contornos do efeito ajustado pelo viés:

```{r sens-contour, eval = FALSE}
...
O contorno vermelho marca o conjunto de $(R^2_{X\sim U}, R^2_{Y\sim U})$
para os quais o efeito ajustado é zero.  Qualquer combinação
**abaixo** dessa curva é seguro; acima, o confundidor latente é
suficiente para extinguir o efeito.
...
- **v1.9.1 (atual)**: `foundation_embed_at_coords()` +
  ...
  decisivo passado.
- **v1.9.2 (atual)**: análise de sensibilidade à la
  @CinelliHazlett2020 via `causal_sensitivity_*` — RV e RV_alpha
  por estimador, bias-contour grid.  Descoberta: o único efeito
  *robusto* no Cerrado sob qualquer estimador é Clay → SOC na OLS
  naïve (RV = 24%); MAP → SOC tem RV ≈ 8% (backdoor), 4% (MoCo-IV).
  + runner `data-raw/causal_sensitivity_run.R`.  Agenda para o
  encoder v2: lift do Stage-1 F (requer `EDAPHOS_IV_REAL_STACK=1`).
