...
## Próximas sessões (v1.8.2 → v1.8.3)

- **v1.8.2**: scripts de publicação do gold-standard v2 no Zenodo
  (DOI próprio) + re-benchmark dos três backends com a versão real.
- **v1.8.3**: integrar voting ponderado (Gemma + Claude) como default
  de `causal_llm_ingest_abstract_voted()`, calibrado pelo κ
  backend-vs-gold derivado do benchmark.
- **v1.8.4**: corpus scaling para 10 000 abstracts reais.
